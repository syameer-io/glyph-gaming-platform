...
/**
 * Voice User Joined Event
 *
 * Broadcast event fired when a user joins a voice channel.
 * Notifies all server members in real-time via WebSocket so the UI can
 * update to show the user in the voice channel participant list.
 *
 * This event is broadcast to the private server channel so only server members
 * receive the notification.
 */
class VoiceUserJoined implements ShouldBroadcast
{
     ...
     *
     * @param User $user The user who joined the voice channel
     * @param Channel $channel The voice channel that was joined
     ...
     * Echo.private('server.123').listen('.voice.user.joined', callback)
     *
     * @return string
     */
